...
<%
	...
	// 로그인 관련 아이디가 있을 경우만 세션 적용. 
		...
		if(type.equals("default")){
			...
			try{
				conn = connect();
				String q = "select * from dbo.e_User where LoginID =? and UserType='default' ";
				...
				if(rs.next()) {
					...
					if(accessID.equals(id) && accessPW.equals(password)){
						...
						if(device.equals("m")){
							...
							out.print("<script>window.location = '" + redirect + "?callback=loginComplete&result=success';</script>");
						}
					} else {
						if(device.equals("m")){
							out.print("loginComplete({'result':'wrong', 'type':'"+type+"', 'id':'"+id+"', 'level':'"+userLevel+"'})");
						}else{
							out.print("<script>window.location = '" + redirect + "?callback=loginComplete&result=wrong';</script>");
						}
					}
				} else {
						...
						if(device.equals("m")) out.print("joinProcess({'result':'goRegist', 'type':'"+type+"', 'id':'"+id+"', 'level':'"+userLevel+"'})");
						else out.print("<script>window.location = '" + redirect + "?callback=joinProcess&result=goRegist';</script>");
			...
			//out.print("3");
				Cookie cookie;
			    cookie = new Cookie("_MEI_TWITTER_ACCESS_TOKEN",null);
			    cookie.setMaxAge(0);
			    response.addCookie(cookie);
			
			
			    
				cookie = new Cookie("_MEI_TWITTER_ACCESS_TOKENSECRET",null);
			    cookie.setMaxAge(0);
			    response.addCookie(cookie);
