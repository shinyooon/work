...
var panelEvent ={
	...
	initEvent : function(){
		...
		$('#btn_total_city').click(function(e){
			e.preventDefault();
			objThis.openPanel($('.panel-srch'));
			setTimeout(function(){
				objThis.openPanel($('.slide-top'));
	...
	initEvent: function(){
		...
		this.$setDateCheck.change(function(){
			...
			if(objThis.range!==null){
				...
				if(objThis.startDate===objThis.endDate){
					objThis.reset();
				}else{
					$('input[name="checkin"]').val(objThis.startDate);
					$('input[name="checkout"]').val(objThis.endDate);

					objThis.$checkIn.val(objThis.getDateTxt(objThis.startDate));
					objThis.$checkOut.val(objThis.getDateTxt(objThis.endDate));
					objThis.$setDateCheck.daterangepicker('option', 'datepickerOptions.maxDate', new Date(objThis.range.start.setDate(objThis.range.start.getDate() + 15)));
				}
			}
		});
