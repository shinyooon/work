...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	 ...
	 * @param {Object} options
	 */
	function buildCalendar(classnameContext, options, setRange_,setStartDate_) {
		var $self,
		...
		function beforeShowDay(date) {
			var dateClass='';
			var yeaderday = new Date();
			yeaderday.setDate(date.getDate() - 1);
			//if(range.start && ((+date === +range.start) || (range.end && range.start <= date && date <= range.end))){
			if((range.end && range.start <= date && date < range.end)){
				dateClass='ui-state-highlight';
			}
			else{
				dateClass=''
			}
			if(yeaderday <= date && date < range.start){
				dateClass='ui-datepicker-unselectable ui-state-disabled';
			}
			if(range.start && date.toDateString() === range.start.toDateString()){
				dateClass='start-date ui-state-highlight';
			}
			if(range.end && date.toDateString() === range.end.toDateString()){
				dateClass='end-date ui-state-highlight';
			}
			if(range.end && (date.toDateString() === range.end.toDateString()) && range.start.toDateString() === range.end.toDateString()){
				dateClass='start-date end-date ui-state-highlight';
			}
			var result = [
					true, // selectable
					dateClass
	...
	function buildDateRangePicker($originalElement, instance, options) {
		...
		function init() {
			...
			calendar = buildCalendar(classname, options, setRange, setStartDate);
		...
		function render() {
			...
			//$('.calendar-area').append($mask).append($container);
			$('.calendar-area').append($container);
		}
		...
		function setStartDate(){
			...
			$originalElement.val(JSON.stringify(formattedRange)).change();
		}
